...
def test_cli_campaign_verify_partial_check_is_incomplete(tmp_path, capsys):
    children = [_child(tmp_path, f"s{i}", i) for i in (0, 1)]
    campaign = build_campaign(children, name="sweep", campaign_dir=tmp_path)
    campaign.runs[1].pop("path")
    write_campaign(campaign, tmp_path)
    ...
    assert "INCOMPLETE" in out
    assert "\nOK " not in out
    assert "one or more child runs not checked" in out
    assert "campaign: 1 of 2 child run(s) loaded and checked" in out
    assert "skipped: runs[1] (cid bafys1)" in out
