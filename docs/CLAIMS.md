...
| L2 | Seed not grindable after the plan | L1 **and** a bound precommit `seed_plan` **and** `precommit.ots` **and** the run's seed equals `sha256-v1` at the recorded index for **exactly** `min_round` **and** that round's randomness authenticates against the beacon. L2 is not scientific correctness; it is “this result was not chosen from an unbound seed search after seeing outcomes.” Publishing a subset of the committed set is allowed; missing indices are listed. | `farm-notary precommit --seed-count N --inclusion …`, `farm-notary derive-seeds`, `farm-notary verify` |
